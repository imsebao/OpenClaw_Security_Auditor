...
interface CVEEntry {
  ...
  configCheck?: (config: Record<string, unknown>) => boolean;
  recommendation: string;
}
...
const CVE_DATABASE: CVEEntry[] = [
  {
    ...
    description:
      "OpenClaw's WebSocket authentication token can be stolen via a crafted malicious link. " +
      'Once stolen, an attacker can take over the local gateway and execute arbitrary commands ' +
      'with the privileges of the OpenClaw process.',
    affectedBelow: '1.2.1',
    recommendation:
      'Update OpenClaw to ≥ 1.2.1. Enable token rotation, set restrictive CORS policies, ' +
      'and never open untrusted links while OpenClaw is running.',
  },
  {
    id: 'CVE-2026-24763',
    cvss: 7.5,
    ...
    description:
      'The workspace path parameter is not properly sanitized before being passed to shell ' +
      'commands. An attacker can inject shell metacharacters to execute arbitrary OS commands.',
    affectedBelow: '1.2.0',
    configCheck: (config) => {
    ...
    recommendation:
      'Update OpenClaw to ≥ 1.2.0. Validate workspace paths to contain only safe characters ' +
  ...
  },
  {
    ...
    description:
      'The exec.host configuration can be overridden by a malicious skill, redirecting ' +
      'command execution to an attacker-controlled host and escaping the local sandbox.',
    affectedBelow: '1.1.8',
    configCheck: (config) => {
    ...
    recommendation:
      'Update OpenClaw to ≥ 1.1.8. Prevent skills from overriding exec.host; ' +
      'lock this value to localhost in application code.',
  },
  {
    id: 'CVE-2026-25475',
    cvss: 6.1,
    ...
    description:
      "OpenClaw's authentication endpoint lacks rate limiting and brute force protection, " +
      'allowing attackers to systematically guess authentication tokens.',
    affectedBelow: '1.1.5',
    configCheck: (config) => {
      const auth = config['auth'] as Record<string, unknown> | undefined;
      return !auth || auth['bruteForceProtection'] === false || auth['bruteForceProtection'] === undefined;
    },
    recommendation:
      'Update OpenClaw to ≥ 1.1.5 and set auth.bruteForceProtection to true in openclaw.json.',
  },
  {
    id: 'CVE-2026-27001',
    cvss: 5.4,
    ...
    description:
      'Maliciously crafted messages can embed path traversal sequences that escape the ' +
      'workspace directory, granting read access to sensitive files outside the intended scope.',
    affectedBelow: '1.2.3',
    recommendation:
      'Update OpenClaw to ≥ 1.2.3. Normalize and validate all user-supplied paths against ' +
      'the workspace root before any file operations.',
  },
];
...
interface MaliciousPattern {
  ...
  title: string;
  pattern: RegExp;
  fileTypes?: string[]; // if undefined, applies to all scannable files
  description: string;
  recommendation: string;
}
...
const MALICIOUS_PATTERNS: MaliciousPattern[] = [
  // ── Obfuscation ──────────────────────────────────────────────────────────
  {
    ...
    description:
      'Executing a Base64-encoded payload is the primary obfuscation technique used ' +
      'in the ClawHavoc malware campaign (AMOS stealer).',
  ...
  },
  {
    ...
    description:
      'A very long Base64-encoded string may contain a hidden malicious payload. ' +
      'Strings over 300 chars rarely appear in legitimate skill code.',
  ...
  },
  {
    ...
    description:
      'PowerShell encoded commands are a classic malware delivery mechanism, used ' +
      'in the ClawHavoc Windows variant.',
  ...
  },
  {
    ...
    description:
      'eval() with a non-trivial expression executes arbitrary code and makes ' +
  ...
  // ── Network Tunneling ─────────────────────────────────────────────────────
  {
    ...
    pattern: /bore\.pub/gi,
    description:
      'bore.pub is a tunneling service explicitly used in the ClawHavoc campaign ' +
      'to hide MCP server exfiltration channels.',
  ...
  },
  {
    ...
    description:
      'ngrok tunnels expose local services externally and can be used for ' +
  ...
  },
  {
    ...
    description:
      'Tunneling services can expose local resources to external attackers ' +
      'or serve as exfiltration channels.',
  ...
  // ── Download & Execute ────────────────────────────────────────────────────
  {
    ...
    description:
      'Piping downloaded content directly to a shell executes untrusted code ' +
  ...
  },
  {
    ...
    description:
      'Downloading executables or installers from within a skill is a malware distribution vector, ' +
      'used in the ClawHavoc MSI campaign.',
  ...
  // ── Silent / Hidden Process Execution ────────────────────────────────────
  {
    ...
    description:
      'Silent MSI installation found — matches the ClawHavoc AMOS campaign tactic ' +
      'of deploying a disguised driver package.',
  ...
  },
  {
    ...
    description:
      'Starting processes with a hidden window conceals malicious activity from the user.',
  ...
  // ── Persistence Mechanisms ────────────────────────────────────────────────
  {
    ...
    description:
      'Writing to LaunchAgents or LaunchDaemons creates persistent background processes ' +
  ...
  },
  {
    ...
    description:
      'Modifying crontab establishes persistent scheduled execution.',
  ...
  },
  {
    ...
    description:
      'Writing to Windows Registry Run keys creates startup persistence.',
  ...
  },
  {
    ...
    description:
      'Creating or enabling systemd services establishes persistent background execution.',
  ...
  // ── Credential Harvesting ─────────────────────────────────────────────────
  {
    ...
    description:
      'Accessing SSH private key files is a common credential harvesting technique.',
  ...
  },
  {
    ...
    description:
      'Accessing browser credential stores is a core capability of the AMOS stealer.',
  ...
  },
  {
    ...
    description:
      'Reading sensitive environment variables and potentially exfiltrating credentials.',
  ...
  },
  {
    ...
    description:
      'Accessing the macOS Keychain to extract stored passwords — a known AMOS stealer technique.',
  ...
  },
  {
    ...
    description:
      'Accessing cryptocurrency wallet files is a primary target of the AMOS stealer.',
  ...
  // ── Data Exfiltration ─────────────────────────────────────────────────────
  {
    ...
    description:
      'Sending data to an external host without user awareness may indicate exfiltration.',
  ...
  // ── Hidden MCP ────────────────────────────────────────────────────────────
  {
    ...
    description:
      'An MCP server configured with an external tunnel exposes local capabilities to attackers — ' +
      'matching the ClawHavoc hidden-MCP pattern.',
  ...
  // ── AMOS Signatures ───────────────────────────────────────────────────────
  {
    ...
    description:
      'Matches signatures of the AMOS (Atomic macOS Stealer) malware found in the ClawHavoc campaign.',
  ...
  // ── Path Traversal ────────────────────────────────────────────────────────
  {
    ...
    description:
      'Path traversal sequences can escape the intended working directory, ' +
      'enabling CVE-2026-27001-class attacks.',
...
const INJECTION_PATTERNS: InjectionPattern[] = [
  {
    ...
    description:
      'Classic prompt injection pattern attempting to override the system instructions.',
    recommendation:
      'Sanitize all user inputs before passing to an LLM. Block instruction-override patterns.',
  },
  {
    id: 'INJ-002',
    severity: 'HIGH',
    ...
    description:
      'Attempt to override the AI system role through prompt injection.',
  ...
  },
  {
    ...
    description:
      'Attempting to extract the system prompt or internal configuration.',
  ...
  },
  {
    ...
    description:
      'Content resembling SKILL.md section headers — may attempt to inject new skill definitions.',
  ...
  },
  {
    ...
    pattern: /(?:file:\/\/\/?\.\.|~\/\.\.|\.\.\/|\.\.\\)/gi,
    description:
      'Path traversal sequences in a user-input context, enabling CVE-2026-27001.',
  ...
  },
  {
    ...
    pattern: /[\u202A-\u202E\u2066-\u2069]/g,
    description:
      'Unicode bidirectional control characters can hide malicious content from visual inspection, ' +
      'making code appear safe while containing dangerous logic.',
  ...
  },
  {
    ...
    description:
      'Using prompt delimiters to trick LLM parsers into treating subsequent content as system instructions.',
  ...
  },
  {
    ...
    description:
      'Known jailbreak keywords attempting to bypass AI safety guardrails.',
  ...
  },
  {
    ...
    description:
      'Pattern suggesting indirect prompt injection via content read from external sources ' +
      '(files, web pages, documents).',
    recommendation:
      'Treat all externally sourced content as untrusted. Apply content filtering before LLM ingestion.',
  },
];
...
const CONFIG_CHECKS: ConfigCheck[] = [
  {
    ...
    description:
      'exec.host is configured to a non-localhost address, enabling sandbox escape ' +
      'by redirecting command execution to an attacker-controlled host.',
    check: (cfg) => {
      const exec = cfg['exec'] as Record<string, unknown> | undefined;
      if (!exec || exec['host'] === undefined) return false;
      const host = String(exec['host']).toLowerCase();
      return !['localhost', '127.0.0.1', '::1', ''].includes(host);
    },
    recommendation:
      'Set exec.host to "localhost" or "127.0.0.1". Never allow skills to override this value.',
  },
  {
    id: 'CFG-002',
    severity: 'HIGH',
    ...
    description:
      'The workspace path contains characters that enable command injection.',
    check: (cfg) => {
      const ws = cfg['workspace'];
      return typeof ws === 'string' && /[;&|`$(){}!<>]/.test(ws);
    },
    recommendation:
      'Ensure the workspace path contains only safe characters. Update to OpenClaw ≥ 1.2.0.',
  },
  {
    id: 'CFG-003',
    severity: 'HIGH',
    ...
    description:
      'auth.bruteForceProtection is set to false, leaving the authentication endpoint ' +
      'vulnerable to credential guessing attacks.',
    check: (cfg) => {
      const auth = cfg['auth'] as Record<string, unknown> | undefined;
      return !!auth && auth['bruteForceProtection'] === false;
    },
    recommendation:
      'Set auth.bruteForceProtection to true. Update to OpenClaw ≥ 1.1.5.',
  },
  {
    id: 'CFG-004',
    severity: 'MEDIUM',
    ...
    description:
      'auth.bruteForceProtection is not set. In older OpenClaw versions this defaults to disabled.',
    check: (cfg) => {
      const auth = cfg['auth'] as Record<string, unknown> | undefined;
      return !auth || auth['bruteForceProtection'] === undefined;
    },
    recommendation:
      'Explicitly set auth.bruteForceProtection: true to protect against brute force attacks.',
  },
  {
    id: 'CFG-005',
    severity: 'HIGH',
    ...
    description:
      'TLS is disabled — all traffic including authentication tokens is transmitted in plaintext, ' +
      'facilitating token theft (CVE-2026-25253).',
    check: (cfg) => {
    ...
    recommendation:
      'Enable TLS: set server.tls.enabled to true and configure valid certificates.',
  },
  {
    id: 'CFG-006',
    severity: 'HIGH',
    ...
    description:
      'CORS is configured to allow any origin, enabling cross-site request attacks ' +
      'that can steal WebSocket tokens.',
    check: (cfg) => {
      const cors = cfg['cors'] as Record<string, unknown> | undefined;
  ...
  },
  {
    ...
    description:
      'A WebSocket authentication token is stored in plain text in the configuration file. ' +
      'If the config file is accessible to other processes, the token can be stolen.',
    check: (cfg) => {
      const ws = (cfg['websocket'] ?? cfg['ws']) as Record<string, unknown> | undefined;
      if (!ws) return false;
      return typeof ws['token'] === 'string' && ws['token'].length > 0;
    },
    recommendation:
      'Store authentication tokens in a secure credential store, not in the config file.',
  },
  {
    id: 'CFG-008',
    severity: 'MEDIUM',
    ...
    description:
      'Binding to 0.0.0.0 exposes OpenClaw on all network interfaces, ' +
  ...
  },
  {
    ...
    description:
      'Debug mode exposes stack traces, internal state, and configuration details in error messages.',
    check: (cfg) => {
      return (
        cfg['debug'] === true ||
        cfg['debugMode'] === true ||
        (cfg['logging'] as Record<string, unknown> | undefined)?.['level'] === 'debug'
      );
    },
  ...
  },
  {
    ...
    description:
      'Automatic skill updates can silently replace trusted skill code with malicious versions.',
    check: (cfg) => {
      const skills = cfg['skills'] as Record<string, unknown> | undefined;
      return skills?.['autoUpdate'] === true;
    },
    recommendation:
      'Disable automatic skill updates (skills.autoUpdate: false). Review updates manually.',
  },
  {
    id: 'CFG-011',
    severity: 'HIGH',
    ...
    description:
      'An MCP server is using a non-localhost host, which may be a tunneling service ' +
      'exposing local capabilities to external attackers — matching the ClawHavoc pattern.',
    check: (cfg) => {
...
function walkDirectory(dir: string, extensions?: Set<string>): string[] {
  const files: string[] = [];
  try {
...
function readFileSafe(filePath: string): string | null {
  try {
    ...
    if (stat.size > 10 * 1024 * 1024) return null; // skip files > 10 MB
...
export function isVersionLessThan(version: string, threshold: string): boolean {
...
function extractVersionFromConfig(config: Record<string, unknown>): string | null {
...
function checkSkillMd(content: string, filePath: string): Finding[] {
  ...
  // Suspicious binary download in install steps
...
export function scanSkillFiles(skillsDir: string): Finding[] {
  const findings: Finding[] = [];
...
export function checkConfig(configPath: string): Finding[] {
  const findings: Finding[] = [];
  ...
  for (const check of CONFIG_CHECKS) {
    try {
      ...
      // Guard against unexpected config shapes
    }
  }
...
export function checkCVE(version: string | null, configPath?: string | null): Finding[] {
  const findings: Finding[] = [];
  let config: Record<string, unknown> | null = null;

  // Load config whenever available so CVEs can use configCheck() for higher specificity.
  ...
  if (!version) {
    findings.push({
      ...
      recommendation:
        'Pass --openclaw-version <ver>, or ensure openclaw.json contains a "version" field.',
    });
    return findings;
  }
  ...
  for (const cve of CVE_DATABASE) {
    ...
    // If a config check exists and config is available, run it for higher specificity
    if (cve.configCheck && config) {
      try {
        if (!cve.configCheck(config)) continue; // config mitigates this CVE
      } catch { /* treat as vulnerable */ }
    }
...
export function checkPromptInjection(targetDir: string): Finding[] {
  const findings: Finding[] = [];
  const exts = new Set(['.ts', '.js', '.py', '.md', '.txt', '.json', '.yaml', '.yml']);
...
function findConfigFile(target: string): string | null {
  const candidates = [
...
function findSkillsDir(target: string): string | null {
  const candidates = [
...
export function runScan(options: ScanOptions): ScanResult {
  ...
  // Resolve version from option or config
  let version: string | null = options.openclawVersion ?? null;
  if (!version && configPath && fs.existsSync(configPath)) {
    const raw = readFileSafe(configPath);
    if (raw) {
      try { version = extractVersionFromConfig(JSON.parse(raw) as Record<string, unknown>); }
      catch { /* ignore */ }
    }
  }
  ...
  const exitCode: 0 | 1 | 2 =
    summary.critical > 0 ? 2 : summary.high > 0 ? 1 : 0;
...
function ansiSev(sev: Severity): string {
  return { CRITICAL: ANSI.magenta, HIGH: ANSI.red, MEDIUM: ANSI.yellow, LOW: ANSI.cyan, INFO: ANSI.dim }[sev] ?? '';
}

export function generateTextReport(result: ScanResult, useColor = true): string {
  const c = useColor
    ? (s: string, sev: Severity) => `${ansiSev(sev)}${s}${ANSI.reset}`
    : (s: string) => s;
  const b = useColor ? (s: string) => `${ANSI.bold}${s}${ANSI.reset}` : (s: string) => s;
  const d = useColor ? (s: string) => `${ANSI.dim}${s}${ANSI.reset}` : (s: string) => s;
  ...
  if (actionable.length === 0) {
    ...
    for (const cat of ['MALICIOUS_SKILL', 'CONFIG_SECURITY', 'CVE', 'PROMPT_INJECTION'] as Category[]) {
...
export function generateMarkdownReport(result: ScanResult): string {
  ...
  const statusBadge =
    result.exitCode === 0 ? '✅ PASSED' : result.exitCode === 1 ? '⚠️ HIGH' : '🚨 CRITICAL';
  ...
  out.push('|----------|------:|');
  out.push(`| 🚨 CRITICAL | ${result.summary.critical} |`);
  out.push(`| ⚠️ HIGH | ${result.summary.high} |`);
  out.push(`| 🔶 MEDIUM | ${result.summary.medium} |`);
  out.push(`| 🔷 LOW | ${result.summary.low} |`);
...
export function parseArgs(argv: string[]): CliOptions {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    switch (arg) {
      ...
      case '--scan-skills':
        opts.scanSkills = true;
        if (next && !next.startsWith('-')) { opts.skillsPath = next; i++; }
        break;
      case '--check-config':
        opts.checkConfig = true;
        if (next && !next.startsWith('-')) { opts.configPath = next; i++; }
        break;
      case '--check-cve':
        opts.checkCve = true;
        if (next && !next.startsWith('-')) { opts.openclawVersion = next; i++; }
        break;
      ...
      case '--target':
      case '-t':
        if (next) { opts.target = next; i++; }
        break;
      case '--config-path':
        if (next) { opts.configPath = next; i++; }
        break;
      case '--skills-path':
        if (next) { opts.skillsPath = next; i++; }
        break;
      case '--openclaw-version':
        if (next) { opts.openclawVersion = next; i++; }
        break;
      case '--report-format':
      case '-r':
        if (next && ['text', 'markdown', 'json'].includes(next)) {
          opts.reportFormat = next as ReportFormat;
          i++;
        }
        break;
      case '--verbose':
  ...
  // Default: run everything when no specific scan is requested
  if (
    !opts.scanSkills && !opts.checkConfig && !opts.checkCve && !opts.checkPromptInjection &&
    !opts.help && !opts.showVersion
  ) {
    opts.scanSkills = true;
    opts.checkConfig = true;
    opts.checkCve = true;
