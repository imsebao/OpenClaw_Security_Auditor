...
exports.AUDITOR_VERSION = void 0;
exports.isVersionLessThan = isVersionLessThan;
exports.scanSkillFiles = scanSkillFiles;
exports.checkConfig = checkConfig;
exports.checkCVE = checkCVE;
...
const CVE_DATABASE = [
    {
        ...
        description: "OpenClaw's WebSocket authentication token can be stolen via a crafted malicious link. " +
        ...
        affectedBelow: '1.2.1',
        recommendation: 'Update OpenClaw to ≥ 1.2.1. Enable token rotation, set restrictive CORS policies, ' +
    ...
    },
    {
        ...
        description: 'The workspace path parameter is not properly sanitized before being passed to shell ' +
        ...
        recommendation: 'Update OpenClaw to ≥ 1.2.0. Validate workspace paths to contain only safe characters ' +
    ...
    },
    {
        ...
        description: 'The exec.host configuration can be overridden by a malicious skill, redirecting ' +
        ...
        recommendation: 'Update OpenClaw to ≥ 1.1.8. Prevent skills from overriding exec.host; ' +
    ...
    },
    {
        ...
        description: "OpenClaw's authentication endpoint lacks rate limiting and brute force protection, " +
    ...
    },
    {
        ...
        description: 'Maliciously crafted messages can embed path traversal sequences that escape the ' +
            'workspace directory, granting read access to sensitive files outside the intended scope.',
        affectedBelow: '1.2.3',
        recommendation: 'Update OpenClaw to ≥ 1.2.3. Normalize and validate all user-supplied paths against ' +
...
const MALICIOUS_PATTERNS = [
    // ── Obfuscation ──────────────────────────────────────────────────────────
    {
        ...
        description: 'Executing a Base64-encoded payload is the primary obfuscation technique used ' +
    ...
    },
    {
        ...
        description: 'A very long Base64-encoded string may contain a hidden malicious payload. ' +
    ...
    },
    {
        ...
        description: 'PowerShell encoded commands are a classic malware delivery mechanism, used ' +
    ...
    },
    {
        ...
        description: 'eval() with a non-trivial expression executes arbitrary code and makes ' +
    ...
    // ── Network Tunneling ─────────────────────────────────────────────────────
    {
        ...
        pattern: /bore\.pub/gi,
        description: 'bore.pub is a tunneling service explicitly used in the ClawHavoc campaign ' +
    ...
    },
    {
        ...
        description: 'ngrok tunnels expose local services externally and can be used for ' +
    ...
    },
    {
        ...
        description: 'Tunneling services can expose local resources to external attackers ' +
    ...
    },
    {
        ...
        description: 'Downloading executables or installers from within a skill is a malware distribution vector, ' +
    ...
    // ── Silent / Hidden Process Execution ────────────────────────────────────
    {
        ...
        description: 'Silent MSI installation found — matches the ClawHavoc AMOS campaign tactic ' +
    ...
    // ── Persistence Mechanisms ────────────────────────────────────────────────
    {
        ...
        description: 'Writing to LaunchAgents or LaunchDaemons creates persistent background processes ' +
    ...
    // ── Hidden MCP ────────────────────────────────────────────────────────────
    {
        ...
        description: 'An MCP server configured with an external tunnel exposes local capabilities to attackers — ' +
    ...
    // ── Path Traversal ────────────────────────────────────────────────────────
    {
        ...
        description: 'Path traversal sequences can escape the intended working directory, ' +
...
const INJECTION_PATTERNS = [
    ...
    },
    {
        ...
        pattern: /[\u202A-\u202E\u2066-\u2069]/g,
        description: 'Unicode bidirectional control characters can hide malicious content from visual inspection, ' +
    ...
    },
    {
        ...
        description: 'Pattern suggesting indirect prompt injection via content read from external sources ' +
...
const CONFIG_CHECKS = [
    {
        ...
        description: 'exec.host is configured to a non-localhost address, enabling sandbox escape ' +
            'by redirecting command execution to an attacker-controlled host.',
        check: (cfg) => {
            const exec = cfg['exec'];
            if (!exec || exec['host'] === undefined)
                return false;
    ...
    },
    {
        ...
        description: 'auth.bruteForceProtection is set to false, leaving the authentication endpoint ' +
    ...
    },
    {
        ...
        description: 'TLS is disabled — all traffic including authentication tokens is transmitted in plaintext, ' +
    ...
    },
    {
        ...
        description: 'CORS is configured to allow any origin, enabling cross-site request attacks ' +
    ...
    },
    {
        ...
        description: 'A WebSocket authentication token is stored in plain text in the configuration file. ' +
    ...
    },
    {
        ...
        description: 'Binding to 0.0.0.0 exposes OpenClaw on all network interfaces, ' +
    ...
    },
    {
        ...
        description: 'An MCP server is using a non-localhost host, which may be a tunneling service ' +
...
function scanSkillFiles(skillsDir) {
    ...
    for (const fp of files) {
        if (path.basename(fp).toUpperCase() !== 'SKILL.MD')
            continue;
    ...
    for (const filePath of files) {
        ...
        if (skipRoots.some((r) => filePath.startsWith(r + path.sep)) && !isSkillMd)
            continue;
    ...
    if (configPath && fs.existsSync(configPath)) {
        ...
        if (raw) {
            try {
                ...
                if (!version)
                    version = extractVersionFromConfig(config);
            }
            catch { /* ignore */ }
        }
    }
...
function runScan(options) {
    ...
    return {
        ...
        version: exports.AUDITOR_VERSION,
...
function generateTextReport(result, useColor = true) {
    const c = useColor
        ? (s, sev) => `${ansiSev(sev)}${s}${ANSI.reset}`
        : (s) => s;
    ...
    if (actionable.length === 0) {
        ...
        for (const cat of ['MALICIOUS_SKILL', 'CONFIG_SECURITY', 'CVE', 'PROMPT_INJECTION']) {
            ...
            for (const f of group) {
                ...
                if (f.cveId)
                    out.push(`  ${d('CVE    :')} ${f.cveId}${f.cvss ? ` (CVSS ${f.cvss})` : ''}`);
                if (f.file)
                    out.push(`  ${d('File   :')} ${f.file}${f.line ? `:${f.line}` : ''}`);
                out.push(`  ${d('Detail :')} ${f.description}`);
                if (f.evidence)
                    out.push(`  ${d('Evidence:')} ${f.evidence}`);
...
function generateMarkdownReport(result) {
    const out = [];
    const statusBadge = result.exitCode === 0 ? '✅ PASSED' : result.exitCode === 1 ? '⚠️ HIGH' : '🚨 CRITICAL';
    ...
    out.push('---');
    out.push('*Generated by [OpenClaw Security Auditor](https://github.com/openclaw-security/openclaw-security-auditor)*');
    return out.join('\n');
}
...
function parseArgs(argv) {
    ...
    if (!opts.scanSkills && !opts.checkConfig && !opts.checkCve && !opts.checkPromptInjection &&
...
const HELP_TEXT = `
OpenClaw Security Auditor v${exports.AUDITOR_VERSION}
...
function main(argv = process.argv.slice(2)) {
    ...
    if (opts.showVersion) {
        process.stdout.write(`OpenClaw Security Auditor v${exports.AUDITOR_VERSION}\n`);
        process.exit(0);
    }
    if (opts.help) {
